const { validateRequest } = require("../helpers/verifyRequest");
...
const weatherCommand = async (req, res) => {
  try {
    ...
    await axios.post(url, weatherData, header).then(result => {
...
const userCommand = async (req, res) => {
  try {
    ...
    await axios.post(url, payload, header).then(result => {
      res.status(200).end();
    });
  } catch (error) {
    console.log("ERROR:", error);
...
module.exports = {userCommand,eventsCommand, weatherCommand, infoCommand, jokeCommand}
